...
struct Nucleotide {
   let input: String
   let transcription: [Character: String] = ["G": "C",
                                             "C": "G",
                                             "T": "A",
                                             "A": "U"]
   ...
   func complementOfDNA() -> String {
      input.reduce("") { output, character in
         guard let char = transcription[character] else { return "XER" }
...
var test = Nucleotide("G").complementOfDNA()
